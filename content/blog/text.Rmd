...
print(model)
```
By default, number of trees is 500 and number of variables tried at each split is 2 in this case. Error rate is 23.57%.
...
Since the error rate is low at mtry =5.
Building final tree with most optimal customizations i.e., ntree=500, mtry=5 . 
```{r}
model2 <- randomForest(Outcome ~ ., data = diabetes_data, ntree = 500, mtry = 5)
print(model2)
```
